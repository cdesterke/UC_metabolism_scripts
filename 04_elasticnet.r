...
### elastic net 0.4 alpha 

fit <- glmnet(X, y, family = "binomial",alpha=0.4)
plot(fit)

cvfit <- cv.glmnet(X, y, family = "binomial",alpha=0.4)
plot(cvfit)
...
ggtitle("") +theme(text = element_text(size = 18))+
